...
final class MySQL implements MySQLInterface
{
    ...
    public function execute(string $query, array $namedArgs = []): Promise
    {
        ...
        return new Promise(function ($resolve, $reject) use ($query, $namedArgs): void {
            if (count($namedArgs) > 0) $query = $this->buildQueryByNamedArgs($query, $namedArgs);
    ...
    private function buildQueryByNamedArgs(string $query, array $namedArgs): string
    {
        foreach ($namedArgs as $key => $value) {
            if (is_string($value)) $query = str_replace($key, sprintf('"%s"', $value), $query);
            if (is_int($value)) $query = str_replace($key, sprintf('%d', $value), $query);
            if (is_double($value)) $query = str_replace($key, sprintf('%f', $value), $query);
            if (is_bool($value)) $query = str_replace($key, sprintf('%s', $value ? 'true' : 'false'), $query);
        }
